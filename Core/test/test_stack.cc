...
TEST_P(stackFixture, Pop) {
    ...
    if (!vector.empty()) {
        for (size_t i = 0; i < vector.size(); ++i) {
            stack.pop();
            if (i < vector.size() - 1) {
                EXPECT_EQ(stack.top(), vector[vector.size() - i - 2]);
            }
            else {
                EXPECT_THROW(stack.top(), std::underflow_error);
            }
...
class StackDyn : public ::testing::TestWithParam<std::pair<core::Stack<int>, std::vector<int>>> {};
...
INSTANTIATE_TEST_SUITE_P(Numbers, StackDyn,
    ::testing::Values(
        std::make_pair(core::Stack<int>(), std::vector<int>{1, 4, 5, 123}),
